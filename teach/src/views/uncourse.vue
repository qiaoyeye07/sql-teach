...
const unselectedCourses = ref([]);
const loading = ref(false);

// 获取未选课程列表（通过当前用户 Sno）
const getUnselectedCourses = async () => {
  loading.value = true;
  try {
    const Sno = username.value; 
    const response = await axios.get('http://localhost:8081/student/uncourses', {
      params: { Sno } 
    });
    unselectedCourses.value = response.data;
  } catch (error) {
    ElMessage.error('获取未选课程失败');
    console.error(error);
  } finally {
    loading.value = false;
  }
...
const handleSelectCourse = async (row) => {
  try {
    const Sno = username.value; 
    const Cno = row.cno; 
    await axios.post('http://localhost:8081/selectCourse', {
      Sno,
      Cno
    });
    ElMessage.success('选课成功');
    // 选课成功后，重新获取未选课程列表（更新界面）
